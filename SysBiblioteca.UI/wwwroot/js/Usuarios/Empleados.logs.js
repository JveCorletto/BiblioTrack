...
function loadActivos() {
    ...
    $.ajax({
        type: 'POST',
        url: api + 'Seguridad/GetEmpleados',
        ...
        success: function (data) {
            if (data.resultado == 1) {
                ...
                $.each(data.datos, function () {
                    ...
                    html += "   <td width='175'>";
                    html += "       <center>";
                    html += "           <button type='button' title='Edición de Empleado' class='btn btn-primary' onclick='getEmpleado(" + this.idUsuario + ", true)' data-toggle='modal' data-target='#staticEmpleado'><i class='fas fa-edit'></i></button>";
                    html += "           <button type='button' title='Desactivar' class='btn btn-danger' onclick='deactivateEmpleado(" + this.idUsuario + ")'><i class='fas fa-times'></i></button>";
                    html += "       </center>";
                    html += "   </td>";
                    html += "</tr>";
...
//Carga los Cargos
function loadCargos(IdCargo, action) {
    var Obj = {
        Token: localStorage.getItem("UserToken"),
        ActualRute: window.location.hash.replace('#', '')
    };
        ...
        success: function (data) {
            ...
            if (data.resultado == 1) {
                if (action) {
                    $("#Cargo").html(null);
                    var html = "";
                    html += "<option value>Elija un Cargo</option>";
                    $.each(data.datos, function () {
                        html += "<option value='" + this.idCargo + "'>" + this.cargo + "</option>";
                    });
                    $("#Cargo").html(html);

                    if (IdCargo > 0) {
                        $("#Cargo").val('' + IdCargo + '').change();
                    }
                    else {
                        $("#Cargo").val('0').change();
                    }
